# Performance optimisation & Monitoring Module for TextToSpeech Generator
...
class PerformanceMonitor {
    ...
    Add-ApplicationLog -Message "Started monitoring operation: $operationName" -Level "DEBUG"
    }
    ...
    Add-ApplicationLog -Message "Operation $operationName completed in $($duration.TotalSeconds.ToString('F2'))s, Memory delta: $($operation.MemoryDeltaMB)MB" -Level "DEBUG"
    ...
    [void] AlertOnThresholds() {
        ...
        if ($currentMetrics.ProcessMemoryMB -gt 1000) {
            Add-ApplicationLog -Message "HIGH MEMORY USAGE: Process using $($currentMetrics.ProcessMemoryMB)MB" -Level "WARNING"
        }
        
        # CPU threshold alerts (if available)
        if ($currentMetrics.CPUUsagePercent -and $currentMetrics.CPUUsagePercent -gt 90) {
            Add-ApplicationLog -Message "HIGH CPU USAGE: $($currentMetrics.CPUUsagePercent)%" -Level "WARNING"
        }
        ...
        if ($recentOperations.Count -gt 0) {
            ...
            if ($avgMemoryDelta -gt 100) {
                Add-ApplicationLog -Message "HIGH MEMORY ALLOCATION RATE: Average $([Math]::Round($avgMemoryDelta, 2))MB per operation" -Level "WARNING"
            }
        }
    }
}
...
function optimise-MemoryUsage {
    <#
    .SYNOPSIS
    optimises memory usage by forcing garbage collection and cleanup
    #>
    param(
    ...
    Add-ApplicationLog -Message "Starting memory optimisation - Current: $([Math]::Round($beforeMemory / 1MB, 2))MB" -Level "INFO"
    ...
    Add-ApplicationLog -Message "Memory optimisation complete - Freed: $([Math]::Round($memoryFreed / 1MB, 2))MB managed, $([Math]::Round($processMemoryFreed / 1MB, 2))MB process" -Level "INFO"
