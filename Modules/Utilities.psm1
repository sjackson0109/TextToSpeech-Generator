...
function Test-SystemRequirements {
    ...
    try {
        $dotNetRelease = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full' -Name Release -ErrorAction SilentlyContinue
        if ($dotNetRelease) {
            $releaseKey = $dotNetRelease.Release
            # Map release keys to versions: https://docs.microsoft.com/en-us/dotnet/framework/migration-guide/how-to-determine-which-versions-are-installed
            switch ($releaseKey) {
                {$_ -ge 528040} { $requirements.DotNetVersion.Current = "4.8"; break }
                {$_ -ge 461808} { $requirements.DotNetVersion.Current = "4.7.2"; break }
                {$_ -ge 461308} { $requirements.DotNetVersion.Current = "4.7.1"; break }
                {$_ -ge 460798} { $requirements.DotNetVersion.Current = "4.7"; break }
                default { $requirements.DotNetVersion.Current = "< 4.7" }
            }
            $requirements.DotNetVersion.Met = $releaseKey -ge 461308  # 4.7.2 or higher
        } else {
            $requirements.DotNetVersion.Current = "Not Detected"
            $requirements.DotNetVersion.Met = $false
        }
    } catch {
        $requirements.DotNetVersion.Current = "Error Checking"
        $requirements.DotNetVersion.Met = $false
    }
...
function Test-InputSanitization {
    ...
    switch ($InputType) {
        ...
        "TextContent" {
            $sanitized = ConvertTo-SafeString -InputObject $Input
            if ($sanitized -ne $Input) {
