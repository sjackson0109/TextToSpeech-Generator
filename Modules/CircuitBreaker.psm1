...
function Initialize-CircuitBreaker {
    ...
    if (-not $script:CircuitBreakers.ContainsKey($ProviderId)) {
        ...
        Write-ApplicationLog -Message "Circuit breaker initialized for $ProviderId" -Level "INFO"
    }
}
    ...
    switch ($breaker.State) {
        ...
        "Open" {
            ...
            if ($timeoutExpired) {
                ...
                Write-ApplicationLog -Message "Circuit breaker for $ProviderId moved to HalfOpen state" -Level "INFO"
    ...
    switch ($breaker.State) {
        ...
        "HalfOpen" {
            ...
            if ($breaker.SuccessCount -ge $breaker.SuccessThreshold) {
                ...
                $breaker.FailureCount = 0
                $breaker.SuccessCount = 0
                Write-ApplicationLog -Message "Circuit breaker for $ProviderId closed after successful recovery" -Level "INFO"
            }
        }
    }
...
function Send-OperationalAlert {
    ...
    Write-ApplicationLog -Message "OPERATIONAL_ALERT: $($alertData | ConvertTo-Json -Compress)" -Level "WARNING"
...
function Start-AdvancedRetry {
    ...
    while ($attempt -le $MaxRetries) {
        try {
            Write-ApplicationLog -Message "[$ProviderId] Attempt $($attempt + 1)/$($MaxRetries + 1)" -Level "DEBUG"
            ...
            if (-not $isRetriable -or $attempt -gt $MaxRetries) {
                Write-ApplicationLog -Message "[$ProviderId] Operation failed permanently: $($_.Exception.Message)" -Level "ERROR"
                throw
            }
            ...
            Write-ApplicationLog -Message "[$ProviderId] Retrying in $($delay)ms (attempt $attempt): $($_.Exception.Message)" -Level "WARNING"
...
function Test-ProviderHealth {
    ...
    Write-ApplicationLog -Message "Provider health [$ProviderId]: $($healthResult.IsHealthy) ($checkSummary)" -Level $logLevel
...
Export-ModuleMember -Variable CircuitBreaker