...
function New-VoiceForgeTTSProvider {
    ...
    $provider.GetAvailableVoices = {
        if ($null -eq $provider.Configuration -or $provider.Configuration.GetType().Name -ne 'Hashtable') {
            Add-ApplicationLog -Module "VoiceForge" -Message "VoiceForge GetAvailableVoices: Configuration is null or not a hashtable, returning demo voices" -Level "WARNING"
        ...
        if (-not $apiKey) {
            Add-ApplicationLog -Module "VoiceForge" -Message "VoiceForge GetAvailableVoices: No config or env var, returning demo voices" -Level "DEBUG"
            return @('Polly','Alice','Tom')
        }
        ...
        try {
            ...
            Add-ApplicationLog -Module "VoiceForge" -Message "VoiceForge GetAvailableVoices: Exception $($_.Exception.Message)" -Level "ERROR"
            return @('Polly','Alice','Tom')
        }
    }
    return $provider
